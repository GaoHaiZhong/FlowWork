...
<%@ attribute name="pag" type="com.ghz.flow.base.util.Page" required="true"%>
<%@ attribute name="paginationSize" type="java.lang.Integer" required="true"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%
    int current = pag.getPageNo() ;
    long begin = Math.max(1, current - paginationSize/2);
    long end = Math.min(begin + (paginationSize - 1), pag.getTotalPages());
...
<div class="pagination pagination-centered" >
    <ul>
        ...
        <% if ((pag.isHasNext() && current != 1) || (current == end && current != 1)){%>
