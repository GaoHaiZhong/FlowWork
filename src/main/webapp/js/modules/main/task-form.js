...
function participants() {
	...
	$('#sendAddParticipantsRequest').click(function() {
		...
		//由于是测试，所以这里还需要修改
		$.post(ctx + '/CandidateOrparticipant/task/participant/add/' + taskId, {
			userId: userIds,
			type: types
		}, function(resp) {
			if(resp == 'success') {
	...
	$('.link-delete').click(function() {
		...
		$.post(ctx + '/CandidateOrparticipant/deleteParticipant/' + taskId, {
...
function candidate() {
	...
	$('#sendAddCandidateRequest').click(function() {
		...
		$.post(ctx + '/CandidateOrparticipant/addCandidate/add/' + taskId, {
...
function subTask() {
	$('.subtask-delete').click(function() {
		...
		alert(ctx + '/subTask/deleteSubTask/' +taskId);
		location.href = ctx + '/subTask/deleteSubTask/' +taskId;
...
function attachment() {
	...
	$('.attachment-delete').click(function() {
		var ele = this;
		if(confirm('确定删除附件？')) {
			$.get(ctx + '/attachment//download/' + $(this).data('id'), function(resp) {
				if(resp == 'true') {
					$(ele).parent().remove();
				} else {
					alert('删除失败！');
...
function delegate() {
    ...
    $('#delegateUserSelect').change(function() {
        ...
        var value = $(this).val();
        if(value) {
            $.post(ctx + '/leaveMail/setDelegateTask/' + taskId, {
