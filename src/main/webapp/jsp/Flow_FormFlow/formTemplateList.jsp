    ...
    <tr>
        <td class="Detail dataContainer" datakey="formTemplateList">
              ...
              <%--  <img width="16" height="16" src="${pageContext.request.contextPath}/style/images/FileType/doc.gif"/>
                <a href="submitUI.html">${ft.name}</a>--%>
                  <c:forEach items="${fromTemplateList}" var="ft">
          <!-- 显示表单模板列表 -->
            <div id="DetailBlock" class="template">
                <img width="16" height="16" src="${pageContext.request.contextPath}/style/images/FileType/doc.gif"/>
                <a href="${pageContext.request.contextPath}/application/submitUI.action?id=${ft.id}&&pdkey=${ft.pdkey}">${ft.name}</a>
            </div>
                  </c:forEach>
        </td>
    </tr>
