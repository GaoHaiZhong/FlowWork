...
<head>
    ...
    <script type="text/javascript">
		var taskId = '${flowBean.taskEntity.id}';
		var processInstanceId = '${flowBean.taskEntity.processInstanceId}';
	</script>
    <script type="text/javascript" src="${ctx }/js/modules/main/events.js"></script>
    <script type="text/javascript" src="${ctx }/js/modules/main/task-form.js"></script>
...
<body>
	...
	<h3>任务办理—[${flowBean.taskEntity.name}]
		<c:if test="${not empty flowBean.taskEntity.processDefinitionId}">
			，流程定义ID：[${flowBean.taskEntity.processDefinitionId}]
		</c:if>
	</h3>
	<!-- 任务属性 -->
	<p class="task-infos text-left">
		<span><i class="icon-calendar"></i>到期日：
			<span class='due-date'>
				<c:if test="${empty flowBean.taskEntity.dueDate}">无到期日</c:if>
				<c:if test="${not empty flowBean.taskEntity.dueDate}">
					<fmt:formatDate value="${flowBean.taskEntity.dueDate}" pattern="yyyy-MM-dd"/>
				</c:if>
			</span>
		...
		<span><i class="icon-flag"></i>优先级：
			<span class='priority'>
				<c:if test="${empty flowBean.taskEntity.priority}">无优先级</c:if>
				<c:if test="${not empty flowBean.taskEntity.priority}">
					<c:if test="${flowBean.taskEntity.priority == 0}">低</c:if>
					<c:if test="${flowBean.taskEntity.priority == 50}">中</c:if>
					<c:if test="${flowBean.taskEntity.priority == 100}">高</c:if>
				</c:if>
			</span>
		...
		<span><i class="icon-calendar"></i>创建日期：
			<fmt:formatDate value="${flowBean.taskEntity.createTime}" pattern="yyyy-MM-dd hh:mm:ss"/>
	...
	<%-- 任务描述 --%>
	<c:if test="${empty flowBean.taskEntity.description}">
		<p class="text-muted">该任务无描述。</p>
	</c:if>
	<c:if test="${not empty flowBean.taskEntity.description}">
		<p class="text-infos">${flowBean.taskEntity.description}</p>
	</c:if>
	<hr/>
    ...
    <div class="row">
        <div class="span4">
            <span><i class="icon-user"></i>拥有人：<span id="owner">${empty flowBean.taskEntity.owner ? '无' : flowBean.taskEntity.owner}</span></span>
        ...
        <div class="span4">
            <span><i class="icon-user"></i>办理人：<span id="assignee">${empty flowBean.taskEntity.assignee ? '无' : flowBean.taskEntity.assignee}</span></span>
        ...
        <div class="span4">
            <%-- 未委派 --%>
            <c:if test="${empty flowBean.taskEntity.delegationState}">
                <span><i class="icon-user"></i>任务委派：<span id="delegateState">${empty flowBean.taskEntity.delegationState ? '无' : flowBean.taskEntity.delegationState }</span></span>
            </c:if>

            <%-- 已委派 --%>
            <c:if test="${not empty flowBean.taskEntity.delegationState}">
                <i class="icon-user"></i>任务委派：
                <span id="delegateState">
                    <c:if test="${flowBean.taskEntity.delegationState == 'PENDING' }">
                        <i class="icon-ok"></i>被委派
                    </c:if>
                    <c:if test="${flowBean.taskEntity.delegationState == 'RESOLVED' }">
    ...
    <div class="row">
        ...
        <div class="span10">
            <ul style="margin-top: 1em;">
                <c:forEach items="${identityLinks}" var="link">
    ...
    <ul class="unstyled">
        <c:forEach items="${subTasks}" var="subTask">
            <li>
                ...
                <c:if test="${subTask.owner == user.loginname && empty subTask.endTime}">
            ...
            <c:forEach items="${attachments}" var="attachment">
                <li>
                    ...
                    <c:if test="${attachment.type == 'url'}">
                        ...
                        <%-- 文件类型的文件 --%>
                    <c:if test="${attachment.type != 'url'}">
                        <a href="${ctx}/attachment/download/${attachment.id}" target="_blank" data-toggle="tooltip" title="${attachment.description}" data-placement="right" class="attachment-item"><i class="icon-file"></i>${attachment.name}</a>
                    </c:if>
    ...
    <hr/>
    <form action="${ctx }/${flowBean.processDefinitionEntity.key}/completeTask/${flowBean.taskEntity.id}" class="form-horizontal" method="post">
        <h4>任务内容</h4>
        <input name="approval"  value="1" id="approval" type="hidden">
      <!--判断是否外置表单-->
        ...
        <div class="control-group">
            <div class="controls">
                ...
                <c:if test="${not empty flowBean.taskEntity.assignee}">
                    <button type="submit" class="btn btn-primary"  value="1"><i class="icon-ok"></i>完成任务</button>
                    <button type="submit" class="btn btn-primary" onclick="$('#approval').val('2')" value="2"><i class="icon-no"></i>不同意</button>
                    <button type="submit" class="btn btn-primary" onclick="$('#approval').val('3')" value="3"><i class="icon-rejuct"></i>退回</button>
                </c:if>
                </c:if>
                <c:if test="${flowBean.taskEntity.name=='销假'}">
                    <a class="btn" href="${ctx}/${flowBean.processDefinitionEntity.key}/xiaoJia/${flowBean.taskEntity.id}">销假</a>
                </c:if>
                <c:if test="${empty flowBean.taskEntity.assignee}">
                    <a class="btn" href="${ctx }/${flowBean.processDefinitionEntity.key}/claimTask/${flowBean.processDefinitionEntity.key}.action?taskId=${flowBean.taskEntity.id}"><i class="icon-ok"></i>签收</a>
                </c:if>
                <c:forEach items="${sessionScope.user.rolerList}" var="roles" >
                    <c:if test="${roles.rolername=='总经理'}">
                        <a class="btn" href="${ctx }/${flowBean.processDefinitionEntity.key}/helpWork.action?taskId=${flowBean.taskEntity.id}"><i class="icon-ok"></i>交给秘书处理</a>
                    </c:if>
                </c:forEach>
            </div>
        </div>
                    ...
                    <td>
                        ...
                        <select class='group' style="width: 130px;display:none;">
                            <c:forEach items="${groups}" var="group">
                                <option value="${group.rolername}">${group.rolername}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
    ...
    <div id="addSubtaskModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="addSubtaskModalLabel" aria-hidden="true">
        ...
        <div class="modal-body">
            <form id="subTaskForm" action="${ctx}/subTask/add/${flowBean.taskEntity.id}" class="form-horizontal" method="post">
                <div class="control-group">
                    <label class="control-label" name="subTaskName">任务名称:</label>
                    <div class="controls">
                        <input type="text" name="taskName" class="required" />
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" name="subTaskName">任务描述:</label>
                    <div class="controls">
        ...
        <div class="modal-body">
            ...
            <div id="my-tab-content" class="tab-content">
                <div id="file" class="tab-pane active">
                    <form id="fileAttachmentForm" action="${ctx}/attachment/addAttachmentFile/new/file.action" class="form-horizontal" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="taskId" value="${flowBean.taskEntity.id}" />
                        <input type="hidden" name="processInstanceId" value="${flowBean.taskEntity.processInstanceId}" />
                        <div class="control-group">
                            <label class="control-label" name="attachmentName">名称:</label>
                            <div class="controls">
                                <input type="text" name="attachmentName" class="required" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" name="attachmentDescription">描述:</label>
                            <div class="controls">
                                <textarea name="attachmentDescription" class="required"></textarea>
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" name="attachmentDescription">文件:</label>
                            <div class="controls">
                ...
                <div id="web" class="tab-pane">
                    <form id="urlAttachmentForm" action="${ctx}/Attachment/addAttachmentUrl/new/url.action" class="form-horizontal" method="post">
                        <input type="hidden" name="taskId" value="${flowBean.taskEntity.id}" />
                        <input type="hidden" name="processInstanceId" value="${flowBean.taskEntity.processInstanceId}" />
                        <div class="control-group">
                            <label class="control-label" name="attachmentDescription">URL:</label>
                            <div class="controls">
